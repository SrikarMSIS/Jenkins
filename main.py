...
class Jenkins_Automation:
	...
	def admin_jobs(self):
		...
		try:
			...
			time.sleep(1)
			self.topModule = input("-- Please provide the file name of the top module without .v extension:\n")
			self.bottomModule = input("-- Please provide the file name of the bottom module without .v extension:\n")
	...
	def copy_vcd_log(self):
		...
		try:
			...
			for file in vcd_files[:1] + log_files[:1]:
				shutil.copy(file, self.currentPath)
